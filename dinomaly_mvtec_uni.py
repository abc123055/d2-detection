...
def train(item_list):
    ...
    # crop_size must be divisible by patch_size (14 or 16)
    if 'dinov3' in encoder_name:
        crop_size = 384  # 384 / 16 = 24
    else:
        crop_size = 392  # 392 / 14 = 28
    ...
    # 8 target layers: for 12-layer models use consecutive [2..9],
    # for deeper models sample 8 evenly-spaced layers from the middle
    num_target = 8
    if depth <= 12:
        target_layers = list(range(2, 2 + num_target))
    else:
        start = depth // 6                    # skip the very early layers
        step = (depth - start) // num_target  # even spacing
        target_layers = [start + i * step for i in range(num_target)]
