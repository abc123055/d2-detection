...
from dinov3 import DINOv3VisionTransformer
...
def load(name):
    ...
    if "dinov3" in name:
        # Load DINOv3 from local directory, e.g. name = 'dinov3_vit_small_16'
        # or name = 'dinov3_vit_small_16:./Dinov3'
        if ':' in name:
            _, model_dir = name.split(':', 1)
        else:
            model_dir = './Dinov3'
        model = DINOv3VisionTransformer.from_pretrained(model_dir)
        return model
